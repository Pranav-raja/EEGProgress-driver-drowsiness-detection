...
Current_Datasets = 'seed_vig/'  # 数据集路径
Epoch = 150  # 训练轮数
CSV_name = 'seed_vig_v1' + '-ACC'  # 设置表格名称 每次开始时设置
...
data_size = 1000  # 数据集样本数(包含训练集和验证集)280 = 4566 = 4110
...
subject_list = ['to', 'f' 'cp']  # 用户列表

# 固定交叉验证：便于调试
train_source_list = np.load("train_source_list1.npy", allow_pickle=True)
train_target_list = np.load("train_target_list1.npy", allow_pickle=True)
validation_target_list = np.load("validation_target_list1.npy", allow_pickle=True)

Cross_Aver_ACC = []  # 交叉验证准确率
es_time = 0  # 预估程序总剩余时间
es_time_2 = 0  # 预估一 cross剩余时间
Start_Task = 1  # 设定起始 Task
Terminal_Task = 2  # 设定结束 Task
